...
def convertir_utm_a_imagen(coordenadas_utm, imagen_objeto):
    ...
    esquina_noroeste = (437687.66, 4471087.73)
    esquina_sureste = (438205.08, 4470929.27)

    # Realizar la transformación de las coordenadas UTM a píxeles:
    x_pixel = (x_utm - esquina_noroeste[0]) / (esquina_sureste[0] - esquina_noroeste[0]) * imagen_objeto.width
    y_pixel = (y_utm - esquina_noroeste[1]) / (esquina_sureste[1] - esquina_noroeste[1]) * imagen_objeto.height
...
def dibujar_punto(coordenadas_utm, imagen_actual):
    ...
    dibujo = ImageDraw.Draw(imagen_actual)
    ...
    dibujo.ellipse([x_pixel - tamaño_punto, y_pixel - tamaño_punto, x_pixel + tamaño_punto, y_pixel + tamaño_punto], fill="red", outline="red")
...
def leer_gps(cola_datos, puerto_serial):
    while True:
        ...
        if 'GPGGA' in linea_gps:
            ...
            print("Datos GPS recibidos:", partes)
...
ruta_imagen = "Fotos/pruebapng.png"
